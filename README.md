...
- PSV uses the same shared borealis UI shell, exposes HTTP receive routes on `ux0:data/localsend/inbox/`, and periodically announces itself for discovery. The shared UI prepares an `outbox/` with a small test file and has a temporary controller send path: `Y` cycles online peers and `X` sends the first selectable outbox file.
- borealis handheld UI is still a shared status screen; the formal device page, transfer list, file picker, receive prompts, and settings pages are pending. The current `X` test sender is a temporary bring-up control.
