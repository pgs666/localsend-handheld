...
- Switch also has a temporary manual send path for protocol testing: put one file in `sdmc:/switch/localsend/outbox/`, then press `X` in the NRO. It defaults to `192.168.31.150:53317`; create `sdmc:/switch/localsend/target.txt` containing `<ip> <port>` to override it.
