...
- Automatically discovered peers are marked offline when they stop refreshing; manual peers from config stay selectable.
...
- PSV uses the same shared borealis UI shell, exposes HTTP receive routes on `ux0:data/localsend/inbox/`, and periodically announces itself for discovery. The shared UI prepares an `outbox/` with a small test file and has temporary controller send controls: send current file, cancel send, cycle peers, and cycle outbox files with platform-specific button labels.
- borealis handheld UI is still a shared status screen; the formal device page, transfer list, file picker, receive prompts, and settings pages are pending. The current controller sender is a temporary bring-up control.
